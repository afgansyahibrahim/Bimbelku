...
add("new package uses configurable 24 hour booking lead", controller.includes("settingInt('booking_lead_hours', 24") && controller.includes("Jadwal paket paling cepat dimulai {$bookingLeadHours} jam dari sekarang."));
add("same-tutor renewal uses configurable 12 hour lead", controller.includes("renewalUsesSameTutors") && controller.includes("settingInt('renewal_booking_lead_hours', 12"));
...
add("renewal test proves a schedule under 24 hours can pass with old tutor", test.includes("collect([0, 2, 4, 6])") && test.includes("preferred_teacher_id") && test.includes("assertCreated()"));
