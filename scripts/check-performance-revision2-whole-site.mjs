...
expect(
  studentLayout.includes("!isDesktop &&") && studentLayout.includes("<MobileBottomNav"),
  "Bottom nav Student masih dimount pada Desktop.",
);
...
expect(
  teacherLayout.includes("!isDesktop &&") && teacherLayout.includes("<MobileBottomNav"),
  "Bottom nav Teacher masih dimount pada Desktop.",
);
...
expect(
  studentController.includes("'learningProgressReports' =>")
    && studentController.includes("->where('student_id', $studentId)")
    && studentController.includes("->orderByDesc('published_at')"),
  "Latest progress report Student belum eager-loaded dan difilter per murid.",
);
