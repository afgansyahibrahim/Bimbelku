...
add("payment backend ignores client price and calculates wallet server-side", orderController.includes("quoteForOrder") && orderController.includes("wallet_balance_changed") && orderController.includes("$externalDue = round(max(") && orderController.includes("(float) $order->amount - $walletAmount - (float) $order->external_received_amount"));
