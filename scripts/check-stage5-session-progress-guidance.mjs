...
expect(manageClasses.includes("Penyelesaian sesi") && manageClasses.includes("hasil belajar wajib diisi setelah sesi diakhiri"), "Tutor belum mendapat panduan penyelesaian sesi yang jelas");
expect(manageClasses.includes("Selesaikan Sesi") && manageClasses.includes("disabled={!item.can_complete}"), "Penyelesaian sesi tutor belum dikunci oleh readiness backend");
...
expect(hub.includes("Langkah 4 dari 5") && hub.includes("Simpan Progress Sesi"), "Form progress tutor belum terhubung jelas ke alur konfirmasi sesi");
expect(hub.includes("Tidak ada perubahan progress materi pada sesi ini") && hub.includes("no_change_reason"), "Sesi review/evaluasi masih memaksa perubahan subbab palsu");
expect(learningSession.includes("'no_material_change' => ['nullable', 'boolean']") && learningSession.includes("Pilih sedikitnya satu subbab yang dibahas atau tandai bahwa sesi tidak mengubah progress materi"), "Backend belum memvalidasi opsi tanpa perubahan materi");
expect(learningSession.includes("$validated['topic_updates'] = []"), "Mode tanpa perubahan materi masih dapat memutasi status subbab dari client stale");
...
expect(teacherCheap.includes('type Scope = "active" | "history"'), "Kelas Murah tutor belum memisahkan Aktif dan Riwayat");
expect(teacherCheap.includes("Sesi yang dilaporkan") && teacherCheap.includes("Riwayat sesi") && teacherCheap.includes("session_id"), "Progress Kelas Murah belum dicatat per sesi");
expect(teacherCheapController.includes("progress_recorded_at") && teacherCheapController.includes("session_number"), "Backend progress Kelas Murah belum mengikat catatan ke sesi");
expect(cheapClassService.includes("'progress_updates' => $hasAccess"), "Riwayat progress Kelas Murah belum dibatasi ke peserta yang berhak");
expect(exists("bimbelku-backend/database/migrations/2026_08_14_210000_add_progress_history_to_cheap_class_sessions.php"), "Migration riwayat progress Kelas Murah belum ada");
