...
if (-not (Test-Path (Join-Path $projectRoot "package.json"))) {
    throw "package.json tidak ditemukan. Jalankan skrip dari paket BimbelKu Tahap 1."
}
...
try {
    ...
    Invoke-Checked "Menguji lint, TypeScript, kontrak API, dan build frontend" {
        npm run check
    }
}
finally {
...
try {
    ...
    Invoke-Checked "Membersihkan cache konfigurasi Laravel" {
        php artisan optimize:clear
    }
    Invoke-Checked "Memeriksa rute autentikasi dan hak akses" {
        php artisan route:list --path=api
    }
    Invoke-Checked "Menjalankan tes fondasi akun dengan SQLite sementara" {
...
Write-Host ""
Write-Host "Tahap 1 lulus. Tes memakai SQLite sementara dan tidak menghapus database MySQL utama." -ForegroundColor Green
