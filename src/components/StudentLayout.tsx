...
import {
  LayoutDashboard, BookOpen, Search, Menu, X, User, 
  History, HelpCircle, Bell, CheckCircle2, Info, MessageSquare
} from "lucide-react";
...
export default function StudentLayout({ children, title }: StudentLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans text-slate-800 selection:bg-blue-100 selection:text-blue-900">
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-30 lg:hidden transition-opacity duration-300" onClick={() => setSidebarOpen(false)} />
      )}

      {/* --- SIDEBAR --- */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-40 w-72 bg-white border-r border-slate-100 transform transition-transform duration-300 ease-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} shadow-2xl lg:shadow-none flex flex-col`}>
        {/* LOGO AREA */}
        <div className="h-24 flex items-center px-8">
            ...
            <button aria-label="Tutup menu" className="lg:hidden ml-auto p-2 bg-slate-50 text-slate-400 rounded-xl hover:bg-slate-100 hover:text-slate-600 transition" onClick={() => setSidebarOpen(false)}>
              <X size={20} />
            </button>
        </div>
        ...
        <nav className="flex-1 px-5 space-y-8 overflow-y-auto custom-scrollbar py-6">
          <div>
              ...
              <div className="space-y-1">
                  ...
                  <NavItem to="/student/find" icon={Search} label="Cari Bimbingan" active={isActive('/student/find') || isActive('/search')} />
                  <NavItem to="/student/my-classes" icon={BookOpen} label="Kelas Saya" active={isActive('/student/my-classes')} />
              </div>
          </div>
          <div>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        {/* HEADER */}
        <header className="h-20 bg-white/80 backdrop-blur-xl border-b border-slate-200/50 flex items-center justify-between px-6 lg:px-10 sticky top-0 z-20 transition-all">
          ...
          <div className="flex items-center gap-5">
             ...
             <div className="relative">
                 ...
                 {showNotifDropdown && (
                     <>
                        ...
                        <div className="absolute right-0 mt-4 w-80 sm:w-96 bg-white rounded-[2rem] shadow-2xl border border-slate-100 z-[101] overflow-hidden animate-in fade-in zoom-in-95 duration-200 origin-top-right ring-1 ring-slate-100">
             ...
             {/* Profile */}
             <div className="flex items-center gap-4 pl-6 border-l border-slate-200 h-8 cursor-pointer group">
            ...
            <div role="dialog" aria-modal="true" aria-label="Detail notifikasi" className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
                ...
                */}
                <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl relative animate-in zoom-in-95 duration-300 overflow-hidden flex flex-col max-h-[90vh]">
                    ...
                    {/* Header: Fixed */}
                    <div className="relative z-10 p-8 pb-4 shrink-0 bg-white">
                    ...
                    {/* Content: Scrollable */}
                    <div className="relative z-10 px-8 overflow-y-auto custom-scrollbar">
                    ...
                    {/* Footer: Fixed */}
                    <div className="relative z-10 p-8 pt-6 shrink-0 bg-white">
                        <button 
                            onClick={() => setSelectedNotif(null)} 
