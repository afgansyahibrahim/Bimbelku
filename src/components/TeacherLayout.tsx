...
export default function TeacherLayout({ children, title }: TeacherLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans text-slate-800">
      {sidebarOpen && ( <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-30 lg:hidden" onClick={() => setSidebarOpen(false)} /> )}

      {/* SIDEBAR */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-40 w-72 bg-white border-r border-slate-100 transform transition-transform duration-300 ease-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} shadow-xl lg:shadow-none flex flex-col`}>
        <div className="h-24 flex items-center px-8 border-b border-slate-50">
             ...
             <button aria-label="Tutup menu" className="lg:hidden ml-auto text-slate-400 hover:text-indigo-600 transition" onClick={() => setSidebarOpen(false)}><X size={20} /></button>
        </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        <header className="h-20 bg-white/80 backdrop-blur-xl border-b border-slate-200/50 flex items-center justify-between px-6 lg:px-10 sticky top-0 z-20 transition-all">
          <div className="flex items-center gap-4">
            <button aria-label="Buka menu" className="lg:hidden p-2.5 bg-white border border-slate-200 text-slate-500 hover:bg-slate-50 rounded-xl shadow-sm transition" onClick={() => setSidebarOpen(true)}><Menu size={20} /></button>
             ...
             <div className="relative">
                 ...
                 {showNotifDropdown && (
                     <>
                        ...
                        <div className="absolute right-0 mt-4 w-80 sm:w-96 bg-white rounded-3xl shadow-2xl border border-slate-100 z-[101] overflow-hidden animate-in fade-in zoom-in-95 duration-200 origin-top-right">
