...
interface Catalog {
  ...
  learning_modes: { value: Exclude<LearningMode, "">; label: string }[];
}
...
export default function SearchPage() {
  ...
  const appliedSubjectQuery = useRef("");
  const requestedSubject = (searchParams.get("subject") || searchParams.get("q") || "").trim();

  const grades = catalog?.grades_by_level[form.education_level] || [];
  const subjectOptions = useMemo<SubjectOption[]>(
    () => (catalog?.subject_options || []).filter((option) => option.is_active !== false),
    [catalog],
  );

  const subjectIsEligible = useCallback((name: string, educationLevel: string, grade = "") => {
    if (!name) return false;
    const option = subjectOptions.find((candidate) => candidate.name === name);
  ...
  const nextStep = () => {
    if (currentStep === 1) {
      if (!form.education_level || !form.subject_name || !form.grade || !form.chapter.trim()) {
        notify.error("Lengkapi jenjang, mata pelajaran, dan materi utama.");
        return;
      }
    }
    if (currentStep === 2 && !form.learning_mode) {
      notify.error("Pilih metode belajar untuk kelas privat.");
      return;
    }
  ...
  useEffect(() => {
    if (!catalog || !requestedSubject) return;
    const requestKey = requestedSubject.toLocaleLowerCase("id-ID");
    if (appliedSubjectQuery.current === requestKey) return;
    appliedSubjectQuery.current = requestKey;

    const selected = subjectOptions.find(
      (option) => option.name.trim().toLocaleLowerCase("id-ID") === requestKey,
    );
    if (!selected) {
      notify.warning("Mata pelajaran tersebut sedang tidak tersedia.");
      return;
    }
  ...
  }, [catalog, requestedSubject, subjectOptions]);
  ...
  return (
    <StudentLayout title="Cari Bimbingan">
      <div className="mx-auto max-w-7xl space-y-8 pb-12">
        ...
        <Link to="/student/kelas-murah" className="group flex items-center justify-between gap-4 rounded-[1.6rem] border border-violet-100 bg-gradient-to-r from-violet-50 via-white to-fuchsia-50 p-5 shadow-sm transition hover-rise-half hover-shadow-md">
          <div className="flex min-w-0 items-center gap-4"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-violet-600 to-fuchsia-600 text-white"><Users size={20} /></span><div className="min-w-0"><p className="text-xs font-black uppercase tracking-widest text-violet-600">Kelas online bersama</p><p className="mt-1 font-black text-slate-900">Butuh kelas dengan harga lebih hemat?</p><p className="mt-1 text-sm text-slate-600">Lihat Kelas Murah yang dibuat admin. Kelas privat tetap dicari melalui form ini.</p></div></div><ArrowRight className="shrink-0 text-violet-600 transition group-hover:translate-x-1" size={20} />
        </Link>
        ...
        <div className="grid items-start gap-7 xl:grid-cols-[1.08fr_.92fr]">
          ...
          >
              ...
              <div className="mt-7 animate-in fade-in slide-in-from-right-3 duration-300">
                <div className="grid gap-5 md:grid-cols-2">
                  <Field label="Jenjang" icon={GraduationCap}>
                    <Select value={form.education_level} onValueChange={(value) => setForm((current) => ({ ...current, education_level: value, grade: "", subject_name: subjectIsEligible(current.subject_name, value) ? current.subject_name : "", chapter: "", subtopic: "" }))}>
                      <SelectTrigger className="h-12 rounded-xl"><SelectValue placeholder="Pilih jenjang" /></SelectTrigger>
                  ...
                  <Field label={educationDetailLabel(form.education_level)} icon={GraduationCap}>
                    <Select value={form.grade} onValueChange={(value) => setForm((current) => ({ ...current, grade: value, subject_name: subjectIsEligible(current.subject_name, current.education_level, value) ? current.subject_name : "", chapter: "", subtopic: "" }))}>
                      <SelectTrigger className="h-12 rounded-xl"><SelectValue placeholder="Pilih kelas" /></SelectTrigger>
                  ...
                  <Field label="Mata pelajaran" icon={BookOpen}>
                    <SubjectCombobox
                      ...
                      grade={form.grade}
                      onChange={(value) => setForm((current) => ({ ...current, subject_name: value, chapter: "", subtopic: "" }))}
                    />
                  </Field>
            ...
            {currentStep === 2 && (
              <div className="mt-7 animate-in fade-in slide-in-from-right-3 duration-300">
                <div className="grid gap-5 md:grid-cols-2">
                  <Field label="Jenis kelas" icon={UserRoundCheck}>
                    <div className="flex min-h-14 items-center justify-center gap-2 rounded-xl border border-indigo-200 bg-indigo-50 px-3 text-sm font-black text-indigo-700"><UserRoundCheck size={17} />Privat</div>
                    <p className="mt-2 text-xs leading-5 text-slate-500">Satu tutor mendampingi satu murid. Kelas bersama tersedia terpisah pada Kelas Murah.</p>
                  </Field>
            ...
            {currentStep === 4 && (
              <div className="mt-7 animate-in fade-in slide-in-from-right-3 duration-300">
                <div className="space-y-3">
                  ...
                  <ReviewRow icon={UserRoundCheck} label="Layanan" value={`Privat · ${form.learning_mode === "online" ? "Online" : "Offline"}`} />
...
function RequestCard({
  ...
  const rating = teacher?.ratings?.length
    ...
    : null;
  const photo = publicMediaUrl(teacher?.teacher_profile?.photo);
