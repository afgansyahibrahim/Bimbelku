...
type PackageSession = { id: number; session_number: number; starts_at: string; ends_at: string; status: string; progress_recorded_at?: string | null; attended_participants_count?: number | null; report_submitted_at?: string | null; admin_review_notes?: string | null; admin_reviewed_at?: string | null; report_revision_count?: number };
type PackageItem = {
  id: number;
  package_code?: string;
  ...
  return <div className="mt-4 space-y-3">{items.map((item) => {
    ...
    return <article key={item.id} className={`overflow-hidden rounded-2xl border ${needsReview ? "border-amber-200 bg-amber-50/30" : "border-slate-200 bg-white"}`}>
      <div className="grid gap-3 p-4 sm:grid-cols-[155px_1fr_auto] sm:items-center">
        ...
        <div><div className="flex flex-wrap items-center gap-2"><h2 className="text-sm font-black text-slate-950">{item.subject_name} · {item.grade}</h2><StatusBadge status={item.session_status} /></div><p className="mt-1 text-xs text-slate-500">Sesi {item.session_number} dari {item.package_session_count} · {item.chapter}</p>{needsReview && <p className="mt-2 text-xs font-black text-amber-700">Laporan tutor siap diperiksa.</p>}</div>
      ...
      {expanded && item.report_submitted_at && <div className="border-t border-slate-100 bg-white p-4">
        <div className="grid gap-3 md:grid-cols-3"><Detail label="Kehadiran" value={`${item.attended_participants_count ?? 0}/${item.confirmed_participant_count} murid hadir`} /><Detail label="Laporan dikirim" value={dateTime(item.report_submitted_at)} /><Detail label="Status" value={statusText[item.session_status] || item.session_status} /></div>
