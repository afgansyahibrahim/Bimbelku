...
type Banner = { id: number; title: string; description?: string; button_text?: string; image_url?: string; audience: string; destination_kind: "internal" | "external"; destination_url: string; sort_order: number; is_active: boolean; starts_at?: string | null; ends_at?: string | null };
...
function Editor({ editor, plans, subjectOptions, close, saved }: { editor: { type: Tab; item: any }; plans: Plan[]; subjectOptions: SubjectOption[]; close: () => void; saved: () => void }) {
    ...
    try {
      ...
      if (editor.type === "banners") {
        ...
        Object.entries(item).forEach(([key, value]) => {
          if (key === "id" || key === "image_url" || value === null || value === undefined) return;
        ...
        response = item.id
          ? await http.put(`/admin/stage-five/${endpoint}/${item.id}`, item)
          : await http.post(`/admin/stage-five/${endpoint}`, item);
      }
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-end justify-center bg-slate-950/65 sm:items-center sm:p-4">
      <form onSubmit={submit} className="max-h-[94vh] w-full overflow-y-auto rounded-t-[2rem] bg-white p-5 shadow-2xl sm:max-w-2xl sm:rounded-[2rem] sm:p-7">
        <div className="flex items-center justify-between"><h2 className="text-2xl font-black text-slate-900">{item.id ? "Ubah" : "Tambah"} {editor.type === "plans" ? "Paket" : editor.type === "promotions" ? "Promo" : editor.type === "banners" ? "Banner" : "Tutorial"}</h2><button type="button" onClick={close} className="rounded-full bg-slate-100 p-2 text-slate-500"><X size={19} /></button></div>
...
function PromoFields({ item, set, plans, subjectOptions }: any) {
  ...
  return <><Field label="Judul promo"><input required className="form-field" value={item.title} onChange={(e) => set("title", e.target.value)} /></Field><Field label="Kode promo"><input className="form-field uppercase" value={item.code || ""} onChange={(e) => set("code", e.target.value.toUpperCase())} placeholder="BELAJAR20" /></Field><Field label="Jenis potongan"><select className="form-field" value={item.discount_type} onChange={(e) => set("discount_type", e.target.value)}><option value="percentage">Persentase</option><option value="fixed">Nominal</option></select></Field><Field label={item.discount_type === "percentage" ? "Persentase (%)" : "Nominal (Rp)"}><input required type="number" min={1} max={item.discount_type === "percentage" ? 100 : undefined} className="form-field" value={item.discount_value} onChange={(e) => set("discount_value", Number(e.target.value))} /></Field><Field label="Maksimal potongan"><input type="number" min={0} className="form-field" value={item.maximum_discount ?? ""} onChange={(e) => set("maximum_discount", e.target.value ? Number(e.target.value) : null)} /></Field><Field label="Minimal transaksi"><input type="number" min={0} className="form-field" value={item.minimum_purchase || 0} onChange={(e) => set("minimum_purchase", Number(e.target.value))} /></Field><Field label="Kuota total"><input type="number" min={1} className="form-field" value={item.total_quota ?? ""} onChange={(e) => set("total_quota", e.target.value ? Number(e.target.value) : null)} /></Field><Field label="Batas per akun"><input required type="number" min={1} className="form-field" value={item.per_user_limit} onChange={(e) => set("per_user_limit", Number(e.target.value))} /></Field><Field label="Mulai tayang"><input type="datetime-local" className="form-field" value={toLocal(item.starts_at)} onChange={(e) => set("starts_at", e.target.value || null)} /></Field><Field label="Selesai tayang"><input type="datetime-local" className="form-field" value={toLocal(item.ends_at)} onChange={(e) => set("ends_at", e.target.value || null)} /></Field><Field label="Deskripsi" wide><textarea className="min-h-24 w-full rounded-2xl border border-slate-200 p-4 text-sm" value={item.description || ""} onChange={(e) => set("description", e.target.value)} /></Field><Field label="Sasaran paket (kosong = semua)" wide><div className="grid gap-2 sm:grid-cols-2">{plans.map((plan: Plan) => <CheckOption key={plan.id} label={plan.name} checked={(item.target_plan_ids || []).includes(plan.id)} onChange={() => toggle("target_plan_ids", plan.id)} />)}</div></Field><Field label="Sasaran jenjang (kosong = semua)" wide><div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{["SD", "SMP", "SMA", "Umum"].map((level) => <CheckOption key={level} label={level} checked={(item.target_levels || []).includes(level)} onChange={() => toggle("target_levels", level)} />)}</div></Field><Field label="Sasaran metode (kosong = semua)" wide><div className="grid grid-cols-2 gap-2"><CheckOption label="Online" checked={(item.target_modes || []).includes("online")} onChange={() => toggle("target_modes", "online")} /><CheckOption label="Offline" checked={(item.target_modes || []).includes("offline")} onChange={() => toggle("target_modes", "offline")} /></div></Field><Field label="Sasaran mapel (Ctrl/Cmd untuk beberapa)" wide><select multiple size={6} className="form-field h-auto" value={item.target_subjects || []} onChange={(e) => set("target_subjects", Array.from(e.currentTarget.selectedOptions, (option) => option.value))}>{subjectOptions.map((subject: SubjectOption) => <option key={subject.id} value={subject.name}>{subject.name}</option>)}</select><button type="button" onClick={() => set("target_subjects", [])} className="mt-2 text-xs font-black text-indigo-600">Gunakan semua mapel</button></Field><Switch label="Khusus murid baru" checked={item.new_students_only} onChange={(value) => set("new_students_only", value)} /><Switch label="Perlu diklaim" checked={item.claim_required} onChange={(value) => set("claim_required", value)} /><Switch label="Promo aktif" checked={item.is_active} onChange={(value) => set("is_active", value)} /></>;
}

function BannerFields({ item, set, image, setImage }: any) {
  return <><Field label="Judul banner"><input required className="form-field" value={item.title} onChange={(e) => set("title", e.target.value)} /></Field><Field label="Teks tombol"><input className="form-field" value={item.button_text || ""} onChange={(e) => set("button_text", e.target.value)} /></Field><Field label="Sasaran"><select className="form-field" value={item.audience} onChange={(e) => set("audience", e.target.value)}><option value="all">Semua</option><option value="student">Murid</option><option value="teacher">Tutor</option><option value="admin">Admin</option></select></Field><Field label="Jenis tujuan"><select className="form-field" value={item.destination_kind} onChange={(e) => set("destination_kind", e.target.value)}><option value="internal">Halaman internal</option><option value="external">Tautan luar</option></select></Field><Field label="Tujuan" wide>{item.destination_kind === "internal" ? <select className="form-field" value={item.destination_url} onChange={(e) => set("destination_url", e.target.value)}>{destinationOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select> : <input required type="url" className="form-field" value={item.destination_url} onChange={(e) => set("destination_url", e.target.value)} placeholder="https://..." />}</Field><Field label="Deskripsi" wide><textarea className="min-h-24 w-full rounded-2xl border border-slate-200 p-4 text-sm" value={item.description || ""} onChange={(e) => set("description", e.target.value)} /></Field><Field label={item.id ? "Ganti gambar (opsional)" : "Gambar banner"} wide><input required={!item.id} type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => setImage(e.target.files?.[0] || null)} className="block w-full rounded-2xl border border-dashed border-slate-300 p-4 text-sm" />{image && <p className="mt-2 text-xs font-bold text-emerald-600">{image.name}</p>}</Field><Field label="Mulai tayang"><input type="datetime-local" className="form-field" value={toLocal(item.starts_at)} onChange={(e) => set("starts_at", e.target.value || null)} /></Field><Field label="Selesai tayang"><input type="datetime-local" className="form-field" value={toLocal(item.ends_at)} onChange={(e) => set("ends_at", e.target.value || null)} /></Field><Field label="Urutan"><input type="number" min={0} className="form-field" value={item.sort_order} onChange={(e) => set("sort_order", Number(e.target.value))} /></Field><Switch label="Banner aktif" checked={item.is_active} onChange={(value) => set("is_active", value)} /></>;
}
...
function BannerCard({ item, edit, remove }: { item: Banner; edit: () => void; remove: () => void }) { return <CardShell active={item.is_active} edit={edit} remove={remove}><div className="relative h-36 overflow-hidden rounded-2xl bg-gradient-to-br from-blue-700 to-violet-700">{item.image_url && <img src={item.image_url} alt="" className="h-full w-full object-cover" />}<div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 to-transparent" /><p className="absolute bottom-3 left-4 right-4 font-black text-white">{item.title}</p></div><p className="mt-3 text-xs font-bold text-slate-500">{item.audience} → {item.destination_url}</p></CardShell>; }
...
function CardShell({ children, active, edit, remove }: { children: React.ReactNode; active: boolean; edit: () => void; remove: () => void }) { return <article className="rounded-[2rem] border border-slate-100 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><span className={`rounded-full px-2.5 py-1 text-[10px] font-black ${active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{active ? "Aktif" : "Nonaktif"}</span><div className="flex gap-1"><button type="button" onClick={edit} className="rounded-xl p-2 text-indigo-600 hover:bg-indigo-50"><Pencil size={16} /></button><button type="button" onClick={remove} className="rounded-xl p-2 text-rose-500 hover:bg-rose-50"><Trash2 size={16} /></button></div></div>{children}</article>; }
...
function TimeSlotManager({ items, changed }: { items: TimeSlot[]; changed: () => Promise<void> }) {
  ...
  return <section className="rounded-[2rem] border border-slate-100 bg-white p-5 shadow-sm sm:p-6"><div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-indigo-50 text-indigo-600"><Clock3 size={20} /></div><div><h3 className="font-black text-slate-900">Pilihan slot 60 menit</h3><p className="text-xs text-slate-500">Murid hanya dapat memilih jam yang aktif di sini.</p></div></div><form onSubmit={add} className="mt-4 grid gap-3 sm:grid-cols-[140px_1fr_auto]"><input required type="time" value={time} onChange={(event) => setTime(event.target.value)} className="form-field" /><input value={label} onChange={(event) => setLabel(event.target.value)} className="form-field" placeholder="Label, contoh: Pagi" /><button disabled={saving} className="rounded-2xl bg-indigo-600 px-4 py-3 text-sm font-black text-white disabled:opacity-50">{saving ? "Menyimpan…" : "Tambah Slot"}</button></form><div className="mt-4 flex flex-wrap gap-2">{items.map((slot) => <div key={slot.id} className={`flex items-center gap-2 rounded-2xl border px-3 py-2 ${slot.is_active ? "border-emerald-200 bg-emerald-50" : "border-slate-200 bg-slate-50 opacity-60"}`}><button type="button" onClick={() => toggle(slot)} className="text-left"><p className="text-sm font-black text-slate-800">{slot.start_time.slice(0, 5)}</p><p className="text-[10px] font-bold text-slate-500">{slot.is_active ? "Aktif" : "Nonaktif"} · {slot.label || "Tanpa label"}</p></button><button type="button" aria-label={`Hapus slot ${slot.start_time}`} onClick={() => remove(slot)} className="rounded-lg p-1 text-rose-500"><Trash2 size={14} /></button></div>)}</div></section>;
}
