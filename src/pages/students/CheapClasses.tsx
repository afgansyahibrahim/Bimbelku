...
import { AlertCircle, ArrowRight, BarChart3, BookOpen, CalendarDays, Clock3, CreditCard, Loader2, Users, Video } from "lucide-react";
...
export default function CheapClasses() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      notify.error(getApiError(error, "Kelas Murah belum dapat dimuat."));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <StudentLayout title="Kelas Murah">
      <div className="mx-auto max-w-7xl space-y-7 pb-12">
        ...
        {loading ? (
          ...
          <div className="rounded-[2rem] border-2 border-dashed border-slate-200 bg-white px-6 py-16 text-center"><BookOpen className="mx-auto text-slate-300" size={40} /><h2 className="mt-4 text-lg font-black text-slate-800">Belum ada Kelas Murah yang dijadwalkan</h2><p className="mt-2 text-sm text-slate-500">Penawaran baru akan tampil otomatis saat pendaftaran dibuka.</p><Link to="/student/packages/new" className="mt-5 inline-flex text-sm font-black text-indigo-600">Cari les privat <ArrowRight className="ml-1" size={16} /></Link></div>
        ) : visibleClasses.length === 0 ? (
          <div className="rounded-[2rem] border-2 border-dashed border-slate-200 bg-white px-6 py-12 text-center"><BookOpen className="mx-auto text-slate-300" size={34} /><p className="mt-3 text-sm font-bold text-slate-500">{scope === "history" ? "Riwayat Kelas Murah masih kosong." : "Tidak ada Kelas Murah aktif."}</p></div>
        ) : (
...
function ClassCard({ item, working, onJoin, onCancel }: { item: CheapClass; working: boolean; onJoin: () => void; onCancel: () => void }) {
  ...
  return <article className="overflow-hidden rounded-[2rem] border border-slate-100 bg-white shadow-sm transition hover-rise-half hover-shadow-lg">
    <div className="border-b border-indigo-50 bg-gradient-to-br from-indigo-50 to-white p-5 sm:p-6">
      <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.18em] text-indigo-600">Kelas Murah · Online</p><h2 className="mt-2 text-xl font-black text-slate-900">{item.subject_name}</h2><p className="mt-1 text-sm font-bold text-slate-600">{item.education_level} · {item.grade}</p></div><span className={`rounded-full px-3 py-1.5 text-xs font-black ${["confirmed", "completed"].includes(item.status) ? "bg-emerald-100 text-emerald-700" : item.status === "cancelled" ? "bg-rose-100 text-rose-700" : "bg-indigo-100 text-indigo-700"}`}>{stateLabel[item.status] || item.status}</span></div>
      <p className="mt-4 rounded-xl bg-white/80 p-3 text-sm font-semibold text-slate-700">{item.chapter}{item.subtopic ? ` · ${item.subtopic}` : ""}</p>
    </div>
    <div className="space-y-4 p-5 sm:p-6">
      <div className="grid gap-3 text-sm sm:grid-cols-2"><Meta icon={CalendarDays} label="Sesi pertama" value={dateTime(item.starts_at)} /><Meta icon={Clock3} label="Paket belajar" value={`${item.session_count} sesi · 1 jam per sesi`} /><Meta icon={Users} label="Kursi terisi" value={`${item.occupied_seat_count}/${item.maximum_participants}`} /><Meta icon={Users} label="Pembayaran terverifikasi" value={`${item.confirmed_participant_count} peserta · minimum ${item.minimum_participants}`} /><Meta icon={CreditCard} label="Sekali bayar" value={money(item.price_per_student)} /></div>
      <div className="rounded-xl bg-indigo-50 p-3 text-xs font-bold leading-5 text-indigo-800">Bayar satu kali untuk seluruh {item.session_count} sesi. Sesi berikutnya tidak memiliki tagihan baru.</div>
      ...
      {item.sessions.length > 1 && <div className="rounded-xl border border-indigo-100 bg-indigo-50/60 p-3"><p className="text-xs font-black text-indigo-900">Jadwal semua sesi</p><div className="mt-2 grid gap-1.5 text-xs text-indigo-800">{item.sessions.map((session) => <p key={session.id}><b>Sesi {session.session_number}</b> · {dateTime(session.starts_at)} · {sessionStateLabel[session.status] || session.status}</p>)}</div></div>}
...
function Meta({ icon: Icon, label, value }: { icon: typeof CalendarDays; label: string; value: string }) {
  return <div className="flex gap-2"><Icon className="mt-0.5 shrink-0 text-indigo-500" size={16} /><div><p className="text-xs font-bold text-slate-400">{label}</p><p className="mt-0.5 font-bold text-slate-700">{value}</p></div></div>;
}
