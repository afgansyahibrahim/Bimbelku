...
export default function TeacherCheapClasses() {
  ...
  const load = useCallback(async () => {
    ...
    } catch (error) { notify.error(getApiError(error, "Kelas Murah belum dapat dimuat.")); }
  ...
  return <TeacherLayout title="Kelas Murah">
    <div className="mx-auto max-w-6xl space-y-7 pb-12">
      <section data-tour="teacher-cheap-hero" className="rounded-[2rem] bg-gradient-to-br from-indigo-700 to-violet-800 px-6 py-8 text-white shadow-xl sm:px-7"><p className="text-xs font-black uppercase tracking-[.2em] text-indigo-100">Kelas online bersama</p><h1 className="mt-3 text-3xl font-black">Kelas Murah yang ditugaskan</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-indigo-100">Kelas aktif dipisahkan dari riwayat. Setelah setiap sesi berakhir, kirim kehadiran, progress per bab, dan catatan. Sesi baru final setelah admin memverifikasi laporan.</p></section>
      ...
      {loading ? <div className="grid min-h-56 place-items-center rounded-[2rem] bg-white"><Loader2 className="animate-spin text-indigo-600" /></div> : classes.length === 0 ? <Empty title="Belum ada Kelas Murah yang ditugaskan." /> : visibleClasses.length === 0 ? <Empty title={scope === "history" ? "Riwayat Kelas Murah masih kosong." : "Tidak ada Kelas Murah aktif."} /> : <div data-tour="teacher-cheap-list" className="grid gap-5">{visibleClasses.map((item) => {
